using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using System.Text.RegularExpressions;
using System.Threading.Tasks;

namespace RA3_Mod_Translater
{
    public static partial class ProtectSpecialPatternsManager
    {
        // Для URL
        private static readonly List<string> UrlPlaceholders = [];
        private static readonly List<string> RulePlaceholders = [];
        private static readonly List<string> SquareBracketPlaceholders = [];

        // Префикс для плейсхолдеров
        private const string UrlPlaceholderFormat = "\uE000UL_{0}\uE001";
        private const string RulePlaceholderFormat = "\uE000RL_{0}\uE001";
        private const string SquareBracketPlaceholderFormat = "\uE000SQU_{0}\uE001";

        // Паттерны
        const string costPowerPattern = @"Cost\s+(\d+)\s+Power";
        const string urlPattern = @"https?://[^\s\""<>\[\]{}|\\^`]+|[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}/[^\s\""<>\[\]{}|\\^`]*";
        const string tagsPattern = @"\$[a-zA-Z_]\w*|\[@[^\]]+\]|\{[^\}]+\}|%[a-zA-Z0-9_]+%";
        const string squarePattern = @"\[[^\[\]]*\]";

        public class TagInfo
        {
            public string Original { get; set; } = "";
            public string Placeholder { get; set; } = "";
        }

        #region Urls
        public static string ProtectUrls(string text)
        {
            UrlPlaceholders.Clear();

            var matches = UrlRegex().Matches(text);

            string result = text;
            int shift = 0;

            foreach (Match match in matches)
            {
                string url = match.Value;
                UrlPlaceholders.Add(url);
                string placeholder = string.Format(UrlPlaceholderFormat, UrlPlaceholders.Count - 1);

                result = result.Remove(match.Index + shift, url.Length)
                              .Insert(match.Index + shift, placeholder);
                shift += placeholder.Length - url.Length;
            }

            return result;
        }

        public static string RestoreUrls(string text)
        {
            string result = text;
            for (int i = 0; i < UrlPlaceholders.Count; i++)
            {
                result = result.Replace(string.Format(UrlPlaceholderFormat, i), UrlPlaceholders[i]);
            }
            return result;
        }
        #endregion

        #region SpecialPatterns
        public static string ProtectSpecialPatterns(string text)
        {
            RulePlaceholders.Clear();

            // Пример правила: Cost \d+ Power → 150 ед. энергии
            return CostPowerRegex().Replace(text, match =>
            {
                string value = match.Groups[1].Value;
                string replacement = $"{value} ед. энергии";
                RulePlaceholders.Add(replacement);
                return string.Format(RulePlaceholderFormat, RulePlaceholders.Count - 1);
            });
        }

        public static string RestoreSpecialPatterns(string text)
        {
            string result = text;
            for (int i = 0; i < RulePlaceholders.Count; i++)
            {
                result = result.Replace(string.Format(RulePlaceholderFormat, i), RulePlaceholders[i]);
            }
            return result;
        }
        #endregion

        #region Tags
        public static (string protectedText, List<TagInfo> tagInfos) ProtectTags(string text)
        {
            var tagInfos = new List<TagInfo>();

            var matches = TagsRegex().Matches(text);
            string result = text;
            int shift = 0;

            foreach (Match match in matches)
            {
                string tag = match.Value;
                string token = $"__NT{Guid.NewGuid().ToString("N")[..8]}__";

                tagInfos.Add(new TagInfo { Original = tag, Placeholder = token });

                result = result.Remove(match.Index + shift, tag.Length)
                              .Insert(match.Index + shift, token);
                shift += token.Length - tag.Length;
            }

            return (result, tagInfos);
        }

        public static string RestoreTags(string text, List<TagInfo> tagInfos)
        {
            string result = text;
            foreach (var info in tagInfos)
            {
                result = result.Replace(info.Placeholder, info.Original);
            }
            return result;
        }
        #endregion

        #region SquareBrackets
        public static string ProtectSquareBrackets(string text)
        {
            SquareBracketPlaceholders.Clear();

            return SquareRegex().Replace(text, match =>
            {
                string original = match.Value;
                SquareBracketPlaceholders.Add(original);
                return string.Format(SquareBracketPlaceholderFormat, SquareBracketPlaceholders.Count - 1);
            });
        }


        public static string RestoreSquareBrackets(string text)
        {
            string result = text;
            for (int i = 0; i < SquareBracketPlaceholders.Count; i++)
            {
                result = result.Replace(string.Format(SquareBracketPlaceholderFormat, i), SquareBracketPlaceholders[i]);
            }
            return result;
        }
        #endregion

        // Regex'ы

        [GeneratedRegex(costPowerPattern)]
        private static partial Regex CostPowerRegex();

        [GeneratedRegex(urlPattern)]
        private static partial Regex UrlRegex();

        [GeneratedRegex(tagsPattern)]
        private static partial Regex TagsRegex();

        [GeneratedRegex(squarePattern)]
        private static partial Regex SquareRegex();
    }
}